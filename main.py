...
def settings_menu():
    ...
    selected_button = 0
    buttons = ["Громкость: " + str(int(volume * 100)) + "%", "Разрешение: " + str(resolution[0]) + "x" + str(resolution[1]), "Назад"]
...
def character_editor():
    ...
    while True:
        ...
        player_surface.fill(player_colors["shirt"], (0, SCALED_TILE_SIZE // 2, SCALED_TILE_SIZE * 2, SCALED_TILE_SIZE))  # Майка
        player_surface.fill(player_colors["pants"], (0, SCALED_TILE_SIZE, SCALED_TILE_SIZE * 2, SCALED_TILE_SIZE))  # Штаны
        player_surface.fill(player_colors["shoes"], (0, SCALED_TILE_SIZE * 1.5, SCALED_TILE_SIZE * 2, SCALED_TILE_SIZE // 2))  # Обувь
...
def confirm_character():
    ...
    while True:
        ...
        # Отрисовка текста
        draw_dialog_text(f"Итак, Игрок, это окончательное решение?", dialog_font, WHITE, screen, 70, SCREEN_HEIGHT - 230)
...
def main_game():
    while True:
        screen.fill(BLACK)
        draw_dialog_text("Основная игра начинается...", dialog_font, WHITE, screen, SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT // 2)
        pygame.display.flip()
